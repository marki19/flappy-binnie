...
// Added deleteDoc so we can erase the old name!
import { doc, getDoc, setDoc, deleteDoc } from "https://www.gstatic.com/firebasejs/12.11.0/firebase-firestore.js"; 
...
window.onload = () => {
  ...
  const renameBtn = document.getElementById("rename-btn"); // The new Rename button!
  ...
  const performCloudLogin = async (playerName) => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        await setDoc(userRef, { username: playerName, lastPlayed: new Date() }, { merge: true });
      }
    } catch (e) {
  ...
  loadAssets(() => {
    ...
    if (!savedName) {
      ...
      if (modalGuest) modalGuest.addEventListener("click", () => {
          if (loginModal) loginModal.classList.add("hidden");
    ...
    setTimeout(() => {
        ...
        if (toggleBgmBtn && game) toggleBgmBtn.innerText = game.settings.bgm ? "🎵 Music: ON" : "🎵 Music: OFF";
        if (toggleSfxBtn && game) toggleSfxBtn.innerText = game.settings.sfx ? "🔊 SFX: ON" : "🔊 SFX: OFF";
        ...
        if (toggleBgmBtn) toggleBgmBtn.addEventListener("click", () => {
            ...
            toggleBgmBtn.innerText = game.settings.bgm ? "🎵 Music: ON" : "🎵 Music: OFF";
            game.applySettings();
        });
  ...
  // --- BUTTON 2: THE NEW RENAME FEATURE ---
  if (renameBtn) {
    renameBtn.addEventListener("click", async () => {
      ...
      try {
        // Step A: Save your current stats under the NEW name
        ...
        // Step B: Clean up by deleting the OLD name from the database
        if (oldName && !oldName.startsWith("Guest_")) {
          await deleteDoc(doc(db, "users", oldName));
        }

        // Step C: Update the game UI instantly
  ...
  window.openLeaderboard = () => {
    ...
    setTimeout(() => { window.lbJustOpened = false; }, 300);

    if (lbSidebar) lbSidebar.classList.add("open");
    if (lbList) lbList.innerHTML = "<div style='text-align:center; padding: 20px; font-weight:bold; color:#543847;'>Establishing link... 📡</div>";
    ...
    if (typeof subscribeToLeaderboard === "function") {
        leaderboardUnsubscribe = subscribeToLeaderboard(50, (data) => {
          ...
          if (data.length === 0) {
            lbList.innerHTML = "<div style='text-align:center; padding: 20px; font-weight:bold;'>No flight data found.</div>";
            return;
          }
          ...
          data.forEach((player, index) => {
            ...
            if (player.username === localStorage.getItem("fb_username")) item.style.background = "#ffce00";
  ...
  window.addEventListener("mousedown", (e) => {
    ...
    if (lbSidebar && lbSidebar.classList.contains("open") && !lbSidebar.contains(e.target)) {
