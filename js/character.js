...
export const CharacterMenu = {
  ...
  saveFace() {
    ...
    // 1. Save the unclipped context state
    fCtx.save();

    // 2. Create the circular clipping mask
    fCtx.beginPath();
    // Use radius 70 (instead of 75) to leave room for the borders
    fCtx.arc(75, 75, 70, 0, Math.PI * 2);
    fCtx.closePath();
    fCtx.clip();

    // 3. Draw the face image inside the clip
    if (this.stream) {
    ...
    // 4. Restore context to remove the clipping constraint
    fCtx.restore();

    // 5. Draw the thick white outline OVER the edge to hide extra pixels
    fCtx.beginPath();
    fCtx.arc(75, 75, 70, 0, Math.PI * 2);
    fCtx.lineWidth = 8; // Adjust this number to make the white border thicker/thinner
    fCtx.strokeStyle = "white";
    fCtx.stroke();

    // 6. Add a thin dark outer border to match the game's retro aesthetic
    fCtx.beginPath();
    fCtx.arc(75, 75, 74, 0, Math.PI * 2);
    fCtx.lineWidth = 2;
    fCtx.strokeStyle = "#543847"; // The dark brown from your UI
    fCtx.stroke();
  ...
  bindEvents() {
    ...
    // ---> NEW: Stop touch events on the slider from panning the mobile screen
    const slider = document.getElementById("zoomSlider");
